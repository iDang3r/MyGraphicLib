...
Engine::Canvas::Canvas(const Point &start, double width, double height) 
...
bool Engine::Canvas::load_from_image(const char* source) 
{
    ...
    zoom_down(Point(1.0, 0.0));

    return true;
}

bool Engine::Canvas::save_to_image(const char* source)
{
    ws("SAVING IMAGE TO:");
    ww(source);
...
bool Engine::Canvas::move(const Point& delta) 
{
...
int Engine::Canvas::get_px_x(double x)
{
    int px_x = visible_part_.width  * (0 + (x - start_.x) / width_);
    px_x += visible_part_.x;

    return px_x;
}

int Engine::Canvas::get_px_y(double y)
{
    int px_y = visible_part_.height * (1 - (y - start_.y) / height_);
    px_y += visible_part_.y;

    return px_y;
}

void Engine::Canvas::draw_line(const Point &begin, const Point &end, const Color &color, int thickness)
{
    thickness >>= 1;
...
void Engine::Canvas::zoom_up(const Point& point)
{
...
void Engine::Canvas::zoom_down(const Point& point)
{
    int point_x = visible_part_.width  * (0 + (point.x - start_.x) / width_);
    int point_y = visible_part_.height * (1 - (point.y - start_.y) / height_);
...
bool Engine::Canvas::handle(const Event_t &event)
{
